            ...
            try:
                ...
                if site.site_type == 1 and site.dg_fuel_system_installed:
                    try:
                        ...
                        dg_unit = DgUnitConsumption.objects.filter(site=int(location_id),created__date_gt=hard_code_date.date())
                        if dg_unit.exists():
    ...
    def on_message(client, userdata, msg):
        ...
        if "recovery" in msg_type:
            if "dailyConsumption" in msg_subtype:
                    ...
                    for i in range(len(dates) - 1):
                        ...
                        if daily_entry.exists():
                            ...
                            daily_entry.update(unit_consumption=daily_unit_consumption,
                                               ...
                                               energy_saved=daily_saving)
                            print(
                                "Updated with {} units consumption and {} units saving.".format(daily_unit_consumption,
                                                                                                daily_saving))
                        else:
                            print("Creating new daily consumption entry for recovery.")
                            DailySiteReading.objects.create(associated_Site=site, aisle_group=aisle_group[0],
                ...
                try:
                    ...
                    for i in range(len(hours) - 1):
                        ...
                        if hourly_entry.exists():
                            ...
                            print("Creating new hourly consumption entry for recovery of {} for {}.".format(
                                aisle_group_id, dateHour))
                            HourlySiteReading.objects.create(associated_Site=site, aisle_group=int(aisle_group_id),
                                                             leg_id=aisle_group_id,
                                                             unit_consumption=hourly_unit_consumption,
                                                             hourly_baseline_value=leg_hourly_baseline,
                                                             energy_saved=hourly_saving,
                ...
                try:
                    data = re.search(
                        "Power_source : (.*); Recovery_hours : (.*); Recovery_load_runtime : (.*)").groups()
                    power_source = SupplyLoadTimeShare.objects.filter(site=site, power_source=int(data[0]))
                    recovery_hours = data[1].split(",")
                    recovery_load_runtime = data[2].split(",")
                    print("This is the load runtime recovery message for {} in {}."
                          .format(power_source[0].get_power_source_display, site))
                    for i in range(len(recovery_hours) - 1):
                        enrty_datetime = datetime.strptime(recovery_hours[i], "%Y-%m-%d %H:%M:%S")
                        runtime_entry = power_source.filter(reading_from=entry_datetime)
                        if recovery_load_runtime[i] == "ERROR404":
                            load_runtime = 0
                        else:
                        ...
                        if runtime_entry.exists():
                            ...
                            print("Creating new entry for recovery of hour {}".format(entry_datetime))
                            SupplyLoadTimeShare.objects.create(site=site, power_source=int(data[0]),
                                                               hourly_run_time=load_runtime,
