...
from wareApp.dg_fuel.sessions import on_event, update_event, off_event
...
def roadcasteAPIUpdated(vehical, start_date, end_date):
    # Normalize inputs into datetimes if needed and delegate to helper
    try:
...
def get_loconav_fuel_data(vehicle_number, dg_start_date, dg_end_date):
    try:
        ...
        sdt = _conv(dg_start_date)
        edt = _conv(dg_end_date)
        val = fetch_loconav_fuel(vehicle_number, sdt, edt)
        return val if val is not None else 0
    except Exception:
        return 0
...
def DG_fuel_for_Roadcaste(site_id, aisle_group, new_unit_consumption, entry_time):
    # Normalize aisle_group input: accept AisleGroup instance or iterable
    site = Site.objects.get(id=site_id)
    aisle_obj = None
    try:
        # if passed a queryset/list or tuple
        if hasattr(aisle_group, "__iter__") and not isinstance(aisle_group, AisleGroup):
            aisle_obj = list(aisle_group)[0] if len(list(aisle_group)) else None
        else:
            aisle_obj = aisle_group
    except Exception:
        aisle_obj = None

    # Only process when we have an aisle_group and it's a DG power source
    if not aisle_obj:
        return

    # valid DG power sources: 1..5 (0 is mains)
    if aisle_obj.power_source == 0:
        # mains-only aisle: finalize active DG run (DG turned off)
        off_event(site, aisle_obj, entry_time)
        return

    # DG-associated aisle: update or start a DG run via session manager (coalescing handled there)
    update_event(site, aisle_obj, new_unit_consumption or 0, entry_time)
            ...
            try:
                ...
                if site.site_type == 1 and site.dg_fuel_system_installed:
                    try:
                        ...
                        # Use the message's entry date (not a hard-coded date)
                        ...
                        if dg_unit.exists():
                            ...
                            # determine provider: prefer explicit `partner_dg_provider`, fall back to numeric-heuristic
                            provider = (
                            ...
                            # If the site uses Roadcaste, allow the Roadcaste-specific handler to run
                            ...
                            if check_dg_status and aisle_group[0].power_source != 0:
                                ...
                                if (
                                    ...
                                    # Provider selection: prefer Roadcaste for numeric IMEI-like ids,
                                    # otherwise use Loconav. Avoid hardcoded site lists.
                                    partner_id = (site.partner_dg_fuel_id or "").strip()
                                    use_roadcast = partner_id.isdigit()
                                    fuel = None
                                    if use_roadcast:
                                        try:
                                            ...
                                            # Roadcaste failing (500/404/etc) should not crash flow
                                            print(
                                                "Roadcaste probe failed for",
                                                partner_id,
                                                "error:",
                                                e,
                                            )
                                        ...
                                        try:
                                            fuel = get_loconav_fuel_data(
                                                partner_id, dg_start_date, dg_end_date
                                            )
                                        except Exception as e:
                                            print(
                                                "Loconav probe failed for",
                                                partner_id,
                                                "error:",
                                                e,
                                            )
                                    ...
                                    # Staleness guard: if the completed DG run is too old, don't blindly accept
                                    # provider values; mark fetch_fuel_data to retry later.
                                    max_age_days = 7
                                    if (
                                        last_record.dg_end_date
                                        and (
                                            datetime.now() - last_record.dg_end_date
                                        ).days
                                        > max_age_days
                                    ):
                                        print(
                                            ...
                                            "is older than",
                                            max_age_days,
                                            "days; skipping immediate fuel update",
                                        )
                                    ...
                                    epoch_time = int(entry_time.timestamp() * 1000)
                                    from wareApp.dg_fuel.sessions import create_dg_unit_consumption
...
def mqtt_client2():
    ...
    def consumption_alarms(site_id, leg_id, current_consumption):
        ...
        try:
            ...
            if count == 0:
                logger.info(
                    f"Not enough daily data to calculate average consumption for site {site_id}."
                )
                return True
            ...
            if max_threshold_value is not None and min_threshold_value is not None:
                ...
                if count > 1:
                    variance = sum(
                        (x - average_consumption) ** 2 for x in unit_consumptions
                    ) / (count - 1)
            ...
            if current_consumption > total_max_consumption:
                logger.warning(
                    f"High consumption alarm triggered for site {site.site_name} (Current: {current_consumption}, Max Allowed: {total_max_consumption})"
                )
                if not AlarmNotifications.objects.filter(
                    site_id=site_id,
                    aisle_group_id=leg_id,
                    Alarm_type=7,
                    created_time__gte=start_of_day,
                ).exists():
                    high_alarm = AlarmNotifications.objects.create(
                        created_by=None,
                        user_level=2,
                    ...
                    logger.info(
                        f"High consumption alarm already generated today for site {site.site_name}."
                    )
            elif current_consumption < total_min_consumption:
                logger.warning(
                    f"Low consumption alarm triggered for site {site.site_name} (Current: {current_consumption}, Min Allowed: {total_min_consumption})"
                )
                if not AlarmNotifications.objects.filter(
                    site_id=site_id,
                    aisle_group_id=leg_id,
                    ...
                    logger.info(
                        f"Low consumption alarm already generated today for site {site.site_name}."
                    )
            else:
                logger.info(
                    f"For site {site.site_name}, the consumption ({current_consumption}) is within normal bounds ({total_min_consumption} - {total_max_consumption})."
                )
            return True
        except Exception as e:
            logger.exception(
                f"Exception in consumption_alarms for site_id {site_id}: {str(e)}"
            )
            return False
