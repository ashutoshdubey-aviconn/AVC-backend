            ...
            try:
                ...
                if site.site_type == 1 and site.dg_fuel_system_installed:
                    try:
                        ...
                        # aisle_group is a queryset; prefer the first aisle object
                        aisle_obj = aisle_group[0] if (hasattr(aisle_group, "__len__") and len(aisle_group) > 0) else None
                        if aisle_obj is None:
                            # nothing to do
                            pass
                        else:
                            # If mains (power_source == 0) => close any active DG run
                            if aisle_obj.power_source == 0:
                                off_event(site, aisle_obj, entry_time)
                            else:
                                # DG-associated aisle: update or start a run via session manager
                                update_event(site, aisle_obj, new_unit_consumption or 0, entry_time)
                    except Exception as err:
                        print("Exception while delegating DG unit handling to dg_session:", err)
