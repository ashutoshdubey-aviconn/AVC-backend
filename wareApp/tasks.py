...
def roadcasteAPIUpdated(vehical, date):
            ...
            try:
                ...
                if site.site_type == 1 and site.dg_fuel_system_installed:
                    try:
                        ...
                        dg_unit = DgUnitConsumption.objects.filter(site=int(location_id))
                        if dg_unit.exists():
                            print("DgUnitConsumption entries are present for aisle : ", aisle_group_id)
                            last_entry = dg_unit.last()
                            check_dg_status = last_entry.is_dg_on
                            if(location_id in [124, 118, 140, 153]):
                                try:
                                ...
                                ...
                            elif check_dg_status and aisle_group[0].power_source != 0:
                                #entry_time = datetime.strptime(msg_time, "%Y-%m-%d %H:%M:%S.%f")
                                epoch_time = int(entry_time.timestamp() * 1000)
