...
class FetchAllCustomersForSuperAdmin(APIView):
    ...
    def post(self, request):
        ...
        site_counts = {
            ...
            for item in Site.objects.filter(customer_id__in=customer_ids).values("customer_id").annotate(
        ...
        if start_date is not None and end_date is not None:
            ...
            daily_readings = daily_readings.filter(reading_for__gte=start_date, reading_for__lte=end_date)
        else:
            current_date = datetime.now()
        ...
        for item in daily_readings.values("associated_Site__customer_id", "reading_for").annotate(
                sum=Sum("energy_saved"),
                baseline=Sum("daily_baseline_value")):
...
class CustomerAllSiteInfo(APIView):
    ...
    def post(self, request):
        ...
        if user_id is None:
            token = request.META.get('HTTP_AUTHORIZATION')
            token = token[6:]
            customer_id = User.objects.only("id", "UserType").get(id=Token.objects.get(key=token).user_id)
        else:
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response({"error": "SiteID must be a valid integer"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            readings = list(
                base_readings
                .filter(reading_for__year=year, reading_for__month=month)
                .values('leg_id')
            ...
            aisle_groups = {
                ...
                for ag in AisleGroup.objects.filter(site_id=site_id, attached_leg_id__in=leg_ids)
            ...
            previous_month_available = any(
                item["month"] == previous_month_date.month and item["year"] == previous_month_date.year
...
class EnergySavingsHourlyExcelDownloadVB(APIView):
    ...
    def get(self, request):
        ...
        if user_type not in [1, 2]:
            return Response({"result": 0, "msg": "Access denied: admin only"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            site_id = data.get("site_id")
            if not site_id:
                return Response({"result": 0, "msg": "site_id is required"}, status=status.HTTP_400_BAD_REQUEST)

            try:
                site_id = int(site_id)
            except ValueError:
                return Response({"result": 0, "msg": "site_id must be an integer"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            from_date_str = (data.get("from_date") or data.get("from") or data.get("date") or "").strip()
            end_date_str = (data.get("end_date") or data.get("to_date") or data.get("to") or data.get("till_date") or "").strip()

            if not from_date_str:
                return Response({"result": 0, "msg": "Date parameter is required (from_date/from/date)"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            def parse_date(d_str):
                for fmt in ("%Y/%m/%d", "%Y-%m-%d", "%d-%b-%Y"):
                    try:
                        ...
                        pass
                raise ValueError(f"Time data '{d_str}' does not match format '%Y/%m/%d', '%Y-%m-%d', or '%d-%b-%Y'")

            try:
                from_date = parse_date(from_date_str)
                end_date = parse_date(end_date_str)
            except ValueError as e:
                return Response({"result": 0, "msg": str(e)}, status=status.HTTP_400_BAD_REQUEST)

            if from_date > end_date:
                return Response({"result": 0, "msg": "from_date cannot be after end_date"}, status=status.HTTP_400_BAD_REQUEST)

            if (end_date - from_date).days > 31:
                return Response({"result": 0, "msg": "Date range cannot exceed 31 days"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not all_leg_ids:
                return Response({"result": 0, "msg": "No data found for the selected site and date range"}, status=status.HTTP_400_BAD_REQUEST)

            str_leg_ids = [str(lid) for lid in all_leg_ids]

            # Fetch AisleGroup names
            aisle_map = dict(AisleGroup.objects.filter(
                virtual_siteID_id=site_id, is_active=True, attached_leg_id__in=str_leg_ids
            ...
            sorted_leg_ids = sorted(all_leg_ids, key=lambda x: aisle_map.get(str(x), str(x)))
            aisle_names = [aisle_map.get(str(leg_id), str(leg_id)) for leg_id in sorted_leg_ids]

            # Fetch HourlySiteReading in bulk
            start_of_range = datetime.combine(target_date_from, datetime.min.time())
            end_of_range_exclusive = datetime.combine(target_date_to + timedelta(days=1), datetime.min.time())
            ...
            baselines = []
            if site.is_live:
                baselines = list(SiteBaseline.objects.filter(
                    associated_site_id=site_id,
            ...
            date_list = [target_date_from + timedelta(days=i) for i in range(total_days)]
            ...
            for d in date_list:
                ...
                for h in range(24):
                    ...
                    baseline_value = round(day_total_baseline / 24.0, 2) if day_total_baseline else 0.0
            ...
            # Build DataFrames
            hourly_headers = [
                "Date", "Hour", "Baseline (KWH)", "Savings (KWH)", "Total Consumption (KWH)"
            ] + aisle_names
...
class MeterDisconnectionListView(APIView):
    def get(self, request):
        queryset = MeterDisconnectionEvent.objects.all().order_by("-detection_window_end", "meter_number", "meter_id")

        logged_site_id = request.query_params.get("logged_site_id") or request.query_params.get("site_id")
        ...
        try:
            ...
            return Response({"status": 400, "msg": "limit and offset must be numbers."}, status=400)

        limit = max(1, min(limit, 500))
        offset = max(0, offset)
        serializer = MeterDisconnectionEventSerializer(queryset[offset:offset + limit], many=True)
...
class DownloadExcelVB(APIView):
    ...
    def post(self, request):
        try:
            ...
            aisle_map = {
                aisle.attached_leg_id: aisle.aisleGroupName
                for aisle in aisles
            }
...
class ConsumptionAlarmsAPI(APIView):
    def post(self, request):
        ...
        try:
            ...
            alarms_qs = AlarmNotifications.objects.filter(site_id=site_id, Alarm_type__in=[7, 8], is_active=True)
            ...
            if start_date_str and end_date_str:
                ...
                alarms_qs = alarms_qs.filter(created_time__gte=start_date, created_time__lte=end_date)
            
            alarms_qs = alarms_qs.order_by('-created_time')
            
            response_data = []
            for alarm in alarms_qs:
                aisle_name = alarm.aisle_group.aisleGroupName if hasattr(alarm, 'aisle_group') and alarm.aisle_group else str(alarm.object_id) if alarm.object_id else ""
                response_data.append({
                    "id": alarm.id,
                    "alarm_type": alarm.get_Alarm_type_display(),
                    "priority": alarm.get_Alarm_priority_display(),
                    "aisle_group": aisle_name,
                    "created_time": alarm.created_time.strftime("%Y-%m-%d %H:%M:%S")
                })
...
class MeterDisconnectionIngestView(APIView):
    ...
    def post(self, request):
        ...
        if not isinstance(payload, list):
            return Response({"status": 400, "msg": "Payload must be object or list."}, status=400)
        ...
        for entry in payload:
            ...
            defaults = {
                ...
                "last_seen_at": parse_datetime(entry.get("last_seen_at") or "") if entry.get("last_seen_at") else None,
                "source_log_path": entry.get("source_log_path") or "",
                "raw_log_line": entry.get("raw_log_line"),
                "synced_to_cloud": True,
                "synced_at": parse_datetime(entry.get("synced_at") or "") if entry.get("synced_at") else None,
