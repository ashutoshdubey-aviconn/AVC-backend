...
class FetchAllCustomersForSuperAdmin(APIView):
    ...
    def post(self, request):
        ...
        site_counts = {
            ...
            for item in Site.objects.filter(customer_id__in=customer_ids)
            .values("customer_id")
            .annotate(
                total_sites=Count("id"),
                total_live_sites=Count("id", filter=Q(is_live=True)),
            )
        }

        daily_readings = DailySiteReading.objects.filter(
            associated_Site__customer_id__in=customer_ids,
        ...
        readings_by_customer = defaultdict(list)
        for item in daily_readings.values(
            "associated_Site__customer_id", "reading_for"
        ).annotate(sum=Sum("energy_saved"), baseline=Sum("daily_baseline_value")):
        ...
        for users in all_users:
            ...
            if total_saved > 0 and total_base > 0:
                ...
                min_saved = min(total_energy_saved)
                max_baseline = total_baseline[total_energy_saved.index(max_saved)]
                min_baseline = total_baseline[total_energy_saved.index(min_saved)]
                if max_baseline > 0:
                    max_saving = (max_saved / max_baseline) * 100
                if min_baseline > 0:
...
class SuperAdminSnapShot(APIView):
    def post(self, request, *args):
        ...
        all_alarms = AlarmNotifications.objects.filter(
            site_id__in=all_live_sites_id, is_active=True
        ).count()
        print(all_alarms)
        counter = 0
        ...
        for users in all_users:
            ...
            if start_date is not None and end_date is not None:
                ...
                print("end_date", end_date)
                d = (
                    DailySiteReading.objects.values("reading_for")
                    .filter(
                        associated_Site__in=all_live_sites_id,
                        reading_for__gte=start_date,
                ...
                current_date = datetime.now()
                d = DailySiteReading.objects.filter(
                    associated_Site__in=all_live_sites_id,
                    reading_for__year=current_date.year,
                    reading_for__month=current_date.month,
                ).aggregate(
                ...
                print("output : ", d)
            if (
                d.get("total_energy_saved") is not None
                and d.get("total_energy_saved") > 0
                and d.get("total_baseline") is not None
                and d.get("total_baseline") > 0
            ):
                counter += 1
                total_percentage_saved += (
                    d.get("total_energy_saved") / d.get("total_baseline")
                ) * 100
        ...
        return Response(
            {
                "current_date": date,
                "alarms": all_alarms,
                "total_unit_consumed": round(total_consumed_energy, 2),
...
class SuperUserAllAlarmsPriorityBasedOnCustomersList(APIView):
    def post(self, request):
        ...
        for customer in all_customers:
            ...
            alarms = (
                AlarmNotifications.objects.values(
                    "Alarm_priority", "site_id__site_name"
                )
                .filter(site_id__in=all_sites_id, is_active=True)
                .annotate(alarm_count=Count("id"))
            )
...
class SuperUserAllAlarmsTypeBasedOnCustomersList(APIView):
    def post(self, request):
        ...
        for customer in all_customers:
            ...
            alarms = (
                AlarmNotifications.objects.filter(
                    site_id__in=all_sites_id, is_active=True
                )
                .annotate(
...
class SiteTotalGraphDataDaily(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            legs = (
                DailySiteReading.objects.filter(associated_Site=site)
            ...
            for m_start, m_end in month_ranges:
                is_past_month = (m_start.year < current_date.year) or (
                    m_start.year == current_date.year
                    and m_start.month < current_date.month
                )
                ...
                if is_past_month:
                    ...
                    if cached_month_data is None:
                        ...
                        if full_month_start.month == 12:
                            full_month_next = full_month_start.replace(
                                year=full_month_start.year + 1, month=1, day=1
                            )
                        else:
                            full_month_next = full_month_start.replace(
                                month=full_month_start.month + 1, day=1
                            )
                        ...
                        for r in month_readings:
                            cached_month_data[r["leg_id"]][r["reading_for"]] = r[
                                "unit_consumption"
                            ]
                    ...
                    fresh_readings = DailySiteReading.objects.filter(
                        associated_Site=site, reading_for__range=[m_start, m_end]
                    ).values("leg_id", "reading_for", "unit_consumption")

                    for r in fresh_readings:
                        readings_map[r["leg_id"]][r["reading_for"]] = r[
                            "unit_consumption"
                        ]
            ...
            name_map = {}
            try:
                aisle_groups = AisleGroup.objects.filter(
                    attached_leg_id__in=legs
                ).values("attached_leg_id", "aisleGroupName")
                name_map = {
                    item["attached_leg_id"]: item["aisleGroupName"]
                    for item in aisle_groups
                }
            except Exception as e:
...
class SiteTotalGraphDataHourly(APIView):
    ...
    def post(self, request):
        ...
        try:
            data = []
            hours = []
            for leg in DailySiteReading.objects.distinct("leg_id").filter(
                associated_Site=site
            ):
                print("leg id is:", leg)
                hourly_consumption = []
                ...
                for i in range(24):
                    ...
                    print("hour :", hour)
                    hours_graph_data = HourlySiteReading.objects.filter(
                        associated_Site=site, leg_id=leg, reading_from=hour
                    )
                    print("hours_graph_data", hours_graph_data)
                    if hours_graph_data.exists():
                        one_hour_consumption = round(
                            hours_graph_data[0].unit_consumption, 2
                        )
    ...
    def post(self, request):
        ...
        try:
            ...
            rooms = list(
                reversed(
                    list(
                        AisleGroup.objects.filter(site=site_id).values_list(
                            "attached_leg_id", flat=True
                        )
                    )
                )
            )
            for i in rooms:
                cons = list(
                    HourlySiteReading.objects.filter(
                        associated_Site=site_id, leg_id=i, reading_from__date=date
                    )
                    .order_by("reading_from__hour")
                    .values_list("unit_consumption", flat=True)
                )
                cons.extend([0] * (24 - len(cons)))
                hourlyData.append(
                    {
                        "name": (
                            AisleGroup.objects.get(attached_leg_id=i).aisleGroupName
                            if AisleGroup.objects.filter(attached_leg_id=i).exists()
                            else i
                        ),
            ...
            return Response(
                {"status": 1, "Hours": hours, "Data": hourlyData, "msg": "Data Present"}
            )
...
class SubmeteringMonthlyBarChart_new(APIView):
    ...
    def post(self, request):
        try:
            ...
            data = request.data
            site_id, from_date, till_date, user_type = (
                data.get("site_id", ""),
                data.get("from_date", ""),
                data.get("till_date", ""),
                data.get("user_type", ""),
            ...
            dates = [
                (from_date + timedelta(days=x)).strftime("%Y-%m-%d")
                for x in range((till_date - from_date).days + 1)
            ]
            dataList = []
            site = Site.objects.get(id=site_id)
            ...
            rooms = AisleGroup.objects.filter(site=site_id).values_list(
                "id", "aisleGroupName"
            )

            # Extracting the Data for the given Date Range
            for room in rooms:
                Dailydata = DailySiteReading.objects.filter(
                    associated_Site=site,
                    leg_id=room[0],
                    reading_for__range=[from_date, till_date],
                    is_visible=True,
                ).order_by("reading_for")
                ...
                if Dailydata.exists:
                    ...
                    new_list = [
                        round(
                            DailydataDict.get(
                                datetime.strptime(d, "%Y-%m-%d").date(), 0
                            ),
                            2,
                        )
                        for d in dates
                    ...
                    dataList.append(
                        {"name": room[1], "data": new_list, "type": "column"}
                    )
                else:
                    Dailydata = [0] * len(dates)
                    dataList.append(
                        {"name": room[1], "data": Dailydata, "type": "column"}
                    )

            # Returning the Response
            return Response(
                {"status": 200, "Dates": dates, "Data": dataList}, status=200
            )
...
class SiteCurrentLoadInfo(APIView):
    ...
    def post(self, request):
        ...
        try:
            active_power_sources = SiteLoadPower.objects.filter(
                Associated_Site=int(site_id), Status="ON"
            )
            ...
            return Response(
                {
                    "Total_Load": str(round(load_value, 3)),
                    "R_Voltage": str(r_volt),
                    "Y_Voltage": str(y_volt),
                    "B_Voltage": str(b_volt),
                    "R_Current": str(round(r_current, 0)),
                    "Y_Current": str(round(y_current, 0)),
                    "B_Current": str(round(b_current, 0)),
                    "R_Power_Factor": str(round(r_power_factor, 3)),
                    "Y_Power_Factor": str(round(y_power_factor, 3)),
                    "B_Power_Factor": str(round(b_power_factor, 3)),
                    "max_load": str(max_load),
                    "min_load": str(min_load),
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if True:  # Token.objects.filter(key=token_key) :
                r_volt = InstantenousPhaseVoltage.objects.filter(
                    Assocted_Site=int(site_id)
                )[0].r_volt
                y_volt = InstantenousPhaseVoltage.objects.filter(
                    Assocaited_Site=int(site_id)
                )[0].y_volt
                b_volt = InstantenousPhaseVoltage.objects.filter(
                    Assocaited_Site=int(site_id)
                )[0].b_volt
                return Response(
                    {
                        "R-phase Voltage": str(r_volt),
                        "Y-phase Voltage": str(y_volt),
                        "B-phase Voltage": str(b_volt),
                    }
                )
...
class SitePhaseCurrentsInfo(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if True:  # Token.objects.filter(key=token_key) :
                r_current = InstantenousPhaseCurrent.objects.filter(
                    Assocaited_Site=int(site_id)
                )[0].r_current
                y_current = InstantenousPhaseCurrent.objects.filter(
                    Assocaited_Site=int(site_id)
                )[0].y_current
                b_current = InstantenousPhaseCurrent.objects.filter(
                    Assocaited_Site=int(site_id)
                )[0].b_current
                return Response(
                    {
                        "R-phase Current": str(r_current),
                        "Y-phase Current": str(y_current),
...
class PowerSupplyInfo(APIView):
    ...
    def post(self):
        ...
        try:
            if True:  # Token.objects.filter(key=token_key) :
                power_supply = PowerSupplyStatus.objects.filter(
...
class ValidatedToken(APIView):
    ...
    def post(self, request):
        ...
        if token:
            ...
            return Response(
                {"error": "invalid API Payload"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(self, request):
        ...
        try:
            if User.objects.filter(username=username):
                ...
                print("user : ", user)
                if str(user.email) == str(email) and str(user.Contact_number) == str(
                    contact_number
                ):
                    print("###")
                    ...
                    return Response(
                        {
                            "result": 1,
                            "msg": "Otp sent successfull . Kindly check your email ",
                            "otp": otp,
                        }
                    )
                else:
                ...
                return Response(
                    {"result": 0, "msg": "Username doesn't match in database"}
                )
        except Exception as err:
...
class ResetPasswordApi(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"result": 0, "msg": "OTP doesn't match"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as err:
            logger.critical(err)
            return Response(
                {"result": 0, "msg": "API Error"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ChangePassword(APIView):
    ...
    def post(self, request):
        ...
        if authenticate(username=request.user.username, password=old_password):
            ...
            return Response(
                {"result": 0, "msg": "Invalid old password"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class CustomerTotalConsumption(APIView):
    ...
    def post(self, request, *args):
        ...
        # customer_id = 2
        customerAllSites = Site.objects.filter(Customer=customer_id).order_by(
            "id"
        )  # no of sites for that customer
        ...
        for i in customerAllSites:
            hourlySiteReading = HourlySiteReading.objects.filter(
                associated_site_id=i.id
            )
        ...
        total_percentage_savings = round(
            (total_energy_savings / total_readings) * 100, 2
        )
        return Response(
            {
                "alarms": alarm,
...
class CustomerAllSiteInfo(APIView):
    ...
    def post(self, request):
        ...
        if user_id is None:
            token = request.META.get("HTTP_AUTHORIZATION")
            token = token[6:]
            customer_id = User.objects.only("id", "UserType").get(
                id=Token.objects.get(key=token).user_id
            )
        else:
            customer_id = User.objects.only("id", "UserType").get(id=int(user_id))
        try:
            if customer_id and customer_id.UserType == 5:
                site_manager = CustomerSiteManager.objects.get(customer=customer_id)
                sites = site_manager.associated_site.all().order_by("id")
                serializer = SiteSerializer(sites, many=True)
                return Response(
                    {"result": 1, "site": serializer.data, "msg": "site manager"}
                )
            else:
                sites = Site.objects.filter(customer=customer_id).order_by("id")
                serializer = SiteSerializer(sites, many=True)
...
class HourlySiteReadingForOneSite(APIView):
    ...
    def post(self, request):
        ...
        totalData = []
        all_leg_id = HourlySiteReading.objects.distinct("leg_id").filter(
            associated_Site=site
        )
        ...
        for i in all_leg_id:
            ...
            for j in range(0, 23):
                ...
                hourly = HourlySiteReading.objects.filter(
                    associated_Site=site,
                    leg_id=i.leg_id,
                ...
                print(
                    "hour is :",
                    hour,
                    "consumption of this hour is : ",
                    unit_consumption,
                )
                oneLegDataList.append(
                    {"hour": hour.hour, "consumption": unit_consumption}
                )
...
class PowerSourceDistributionMonthlyBarChartTime(APIView):
    def post(self, request):
        ...
        if tillDate.month == currentDate.month:
            ...
            for power_source in (
                SupplyLoadTimeShare.objects.distinct("power_source")
                .order_by("power_source")
                .filter(site=site_id)
            ):
                ...
                for i in range(30):
                    ...
                    print("date : ", date)
                    graph = SupplyLoadTimeShare.objects.filter(
                        site=site_id,
    ...
    def post(self, request):
        ...
        try:
            ...
            for leg in DailySiteReading.objects.distinct("leg_id").filter(
                ...
                dateUpperLimitCheck = till_date.date()
                print("dateUpperLimitCheck", dateUpperLimitCheck)
                given_date_graph_data = DailySiteReading.objects.filter(
                    associated_Site=site,
                    leg_id=leg,
                    reading_for__gte=dateLowerLimitCheck,
                    reading_for__lte=dateUpperLimitCheck,
                ...
                for i in range(31):
                    if date <= till_date.date():
                        ...
                        if given_date_graph_data.filter(reading_for=date).exists():
                            one_day_consumption = round(
                                given_date_graph_data.filter(reading_for=date)[
                                    0
                                ].unit_consumption,
                                2,
                            )
...
class PowerDistributionHourlyBarChart(APIView):
    ...
    def post(self, request):
        ...
        try:
            data = []
            hours = []
            for leg in HourlySiteReading.objects.distinct("leg_id").filter(
                associated_Site=site
            ):
                print("leg id is:", leg)
                hourly_consumption = []
...
class CustomerSnapshotApiOld(APIView):
    ...
    def post(self, request):
            ...
            if user_type == 1:
                ...
                if int(customer_id) == 27:  # for fan separate site
                    ...
                    aisle_group = AisleGroup.objects.filter(
                        Q(site_id=i.id) & Q(is_visible=True)
                        | Q(attached_leg_id__in=[259, 260, 261])
                    )
                else:
                    print("for other customers")
                    aisle_group = AisleGroup.objects.filter(
                        site_id=i.id, is_visible=True
                    )
                all_leg_id = [aisle.attached_leg_id for aisle in aisle_group]
                daily = DailySiteReading.objects.filter(
                    associated_Site=i.id,
        ...
        try:
            percentageSaved = (
                str(round(energySaved * 100 / (energyConsumed + energySaved), 1)) + " %"
            )
        except Exception as e:
            print("exception is", e)
...
class TotalAlarmsOnCustomerPageTabularForm(APIView):
    ...
    def post(self, request):
        ...
        for i in all_sites:
            ...
            for j in all_alarms:
                alarm = AlarmNotifications.objects.filter(
                    created_by=user, site_id=i, user_level=4, Alarm_type=j.Alarm_type
                )
                if alarm.exists():
...
class TotalAlarmsTypeCountOnCustomerPage(APIView):
    ...
    def post(self, request):
        ...
        user_id = data.get("id", "")
        user = User.objects.get(id=user_id)
        all_alarms = AlarmNotifications.objects.filter(created_by=user).distinct(
            "Alarm_type"
        )
        data_list = []
        alarm_name_list = []
        ...
        for i in all_alarms:
            ...
            alarm_count = AlarmNotifications.objects.filter(
                created_by=user, Alarm_type=i.Alarm_type
            ).count()
            print("alarm count: ", alarm_count)
            data_list.append(
                {"name": alarm_name, "data": [alarm_count], "type": "column"}
            )
            alarm_name_list.append(alarm_name)
            # alarm_count_list.append(alarm_count)
        return Response(
            {"result": 1, "Data": data_list, "alarm_name_list": alarm_name_list}
        )
        ...
        if site.live_date:
            ...
            if user_type == 1:
                if int(data.get("site_id", "")) == 34:
                    ...
                    aisle_group = AisleGroup.objects.filter(
                        site_id=site_id, is_visible=True
                    )
                # aisle_group = AisleGroup.objects.filter(site_id=site_id)
                all_leg_id = [aisle.attached_leg_id for aisle in aisle_group]
                daily = DailySiteReading.objects.filter(
    ...
    def post(self, request):
        ...
        if tillDate.month == currentDate.month:
            if user_type == 1:
                if int(data.get("site_id", "")) == 34:
                    aisle_group = AisleGroup.objects.filter(
                        attached_leg_id__in=[259, 260, 261, 276]
                    )
                else:
                    aisle_group = AisleGroup.objects.filter(site_id=site_id)
            else:
            ...
            for aisle in aisle_group:
                ...
                daily_data = DailySiteReading.objects.filter(
                    associated_Site=site, leg_id=leg, is_visible=True
                )
                for i in range(30):
                    ...
                    if currentDate.date() == date.date():
                        if (
                            site.is_live
                            and user_type in [4, 5]
                            and live_date.date() <= date.date()
                        ) or user_type == 1:
                            hourly = HourlySiteReading.objects.filter(
                                associated_Site=site,
                                leg_id=leg,
                ...
                if aisle:
                    ...
                    name = leg
                dataList.append(
                    {"name": name, "data": unitConsumptionList, "type": "column"}
                )
                savingDataList.append(
                    {"name": name, "data": savingConsumptionList, "type": "column"}
                )
            if site.is_live:
                if int(data.get("site_id", "")) == 34:
                    baseline = SiteBaseline.objects.filter(
                        associated_site_id=site_id, leg_id__in=[259, 260, 261, 276]
                    )
                else:
                ...
                if baseline.exists():
                    if int(data.get("site_id", "")) == 34:
                        latest_baseline_value = SiteBaseline.objects.filter(
                            associated_site_id=site,
                            aisle_group__in=[259, 260, 261, 276],
                        )
                    elif int(data.get("site_id", "")) == 29:
                        latest_baseline_value = SiteBaseline.objects.filter(
                            associated_site_id=29
                        ).exclude(Q(aisle_group__in=[259, 260, 261, 276]))
                    else:
                        latest_baseline_value = SiteBaseline.objects.filter(
                            associated_site_id=site
                        )
                    ...
                    for i in range(30):
                        date = tillDate - timedelta(days=29 - i)
                        baseline_value = latest_baseline_value.filter(
                            Q(baseline_to__gte=date.date())
                            | Q(
                                Q(baseline_from__lte=date.date())
                                & Q(baseline_to__isnull=True)
                            )
                        ).aggregate(Sum("baseline_value"))
                        try:
                ...
                dataList.append(
                    {"name": "baseline", "data": baselineList, "type": "spline"}
                )
        else:
            total_days = (tillDate.day - fromDate.day) + 1
            if user_type == 1:
...
class DemoApi(APIView):
    ...
    def get(self, request):
        ...
        for i in daily:
            my_list.append(
                {
                    "consumption": i.unit_consumption,
                    "saving": i.energy_saved,
                    "date": i.reading_for,
                }
            )
...
class EnergySavingMonthlyTrendApi(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Invalid or missing site_id"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if site_id_int == 34:
            ...
            aisle_group = AisleGroup.objects.filter(
                virtual_siteID=query_site_id, is_visible=True
            )
        ...
        for i in range(11, -1, -1):
            ...
            is_current_month = (
                month.month == current_month and month.year == current_year
            )
            cache_key_data = (
...
class EnergySavingMonthlyTrendExcelDownload(APIView):
    def post(self, request):
        ...
        try:
                ...
                return Response(
                    {
                        "status": 500,
                        "data": [],
                        "msg": "Missing required parameters: site_id, from_month, to_month",
                    }
                )
            ...
            if int(data.get("site_id", "")) == 34:
                baseline_date = (
                    datetime.now().replace(day=12, month=4, year=2023).date()
                )
            else:
                baseline_date = (
                    site.baseline_date.date()
                    if site.baseline_date
            ...
            while current_month <= end_date:
                ...
                if user_type == 1:
                    if int(data.get("site_id", "")) == 34:
                        ...
                        aisle_group = AisleGroup.objects.filter(
                            virtual_siteID=site_id, is_visible=True
                        )
                else:
                    if int(data.get("site_id", "")) == 34:
                        aisle_group = AisleGroup.objects.filter(
                ...
                energy_saved_list.append(round(energySaved, 1))
                carbon_list.append(round(carbon_saved, 1))
                baseline_value_list.append(
                    round(energySaved + energyConsumed, 1)
                )  # Total baseline = energySaved + energyConsumed
            ...
            # Generate filename
            file_name = (
                f"{site_name}_saving_monthly_trend_{from_month}_to_{to_month}.csv"
            )
            ...
            if site:
                for i in range(11, -1, -1):
                    ...
                    if month_year != current_month_key and cached_prev_data:
                        ...
                        month_list.append(cached_prev_data["months"][idx])
                        energy_consumed_list.append(
                            cached_prev_data["energyConsumed"][idx]
                        )
                        energy_saved_list.append(cached_prev_data["energySaved"][idx])
                        carbon_list.append(cached_prev_data["carbon_saved"][idx])
                        percentage_saved_list.append(
                            cached_prev_data["percentageSaved"][idx]
                        )
                        continue
                        ...
                        if previous_date.month == month.month:
                            ...
                            if daily.exists():
                                for j in daily:
                                    ...
                                    if carbon_visible:
                                        carbon_saved = (
                                            energySaved * carbon_emission_value
                                        )
...
class EnergySavingsHourlyBarChartOld(APIView):
    ...
    def post(self, request):
        ...
        dataList = []
        savingList = []
        all_leg_id = DailySiteReading.objects.filter(
            associated_Site=site, reading_for=date.date()
        ).distinct("leg_id")
        print("all leg ids", all_leg_id)
        print("length : ", len(all_leg_id))
        oneDayData = HourlySiteReading.objects.filter(
            associated_Site=site_id, reading_from__date=date.date(), is_visible=True
        )
        for leg in all_leg_id:
            aisle_name = AisleGroup.objects.filter(
                site=site_id, attached_leg_id=leg.leg_id
            )
            ...
            for i in range(24):
                ...
                energy_saved = 0.0
                if aisle_name.exists():
                    hourly = oneDayData.filter(
                        aisle_group=aisle_name[0], reading_from__hour=current_hour
                    )
                else:
                    hourly = oneDayData.filter(
                        leg_id=leg, reading_from__hour=current_hour
                    )
        ...
        if site.is_live:
            ...
            if baseline.exists():
                ...
                for i in range(24):
                    ...
                    try:
                        baseline_value = HourlySiteReading.objects.filter(
                            associated_Site=site,
                        ...
                        if baseline_value.exists():
                            ...
                            print(
                                "baseline doesn't exists for that hour. Sending 0.0 value",
                                baseline_value,
                            )
                            baseline_value = 0.0
                            baselineList.append(baseline_value)
        ...
        return Response(
            {"result": 1, "Hours": hourList, "Data": dataList, "SavingData": savingList}
        )
...
class EnergySavingsHourlyBarChart(APIView):
    ...
    def post(self, request):
        ...
        for aisle in aisle_names:
            ...
            savingConsumptionList = []
            hourly_data = (
                oneDayData.filter(Q(leg_id=leg_id))
                .values("reading_from__hour")
                .annotate(
                    unit_consumption_sum=Sum("unit_consumption"),
                    energy_saved_sum=Sum("energy_saved"),
                )
            )
            hourly_data_dict = {
                reading["reading_from__hour"]: reading for reading in hourly_data
            }

            for i in range(24):
                hourly = hourly_data_dict.get(i)
                unit_consumption = (
                    round(hourly["unit_consumption_sum"], 2) if hourly else 0.0
                )
            ...
            dataList.append(
                {"name": aisle_name, "data": unitConsumptionList, "type": "column"}
            )
            savingList.append(
                {"name": aisle_name, "data": savingConsumptionList, "type": "column"}
            )
        if site.is_live:
            latest_baseline_value = (
                SiteBaseline.objects.filter(associated_site_id=site)
                .exclude(Q(baseline_to__isnull=False))
                .aggregate(Sum("baseline_value"))
            )
            latest_baseline_value = latest_baseline_value["baseline_value__sum"]
            if (
                datetime.now().date() == date.date()
                and latest_baseline_value is not None
            ):
                baselineList = [round(latest_baseline_value / 24, 2) for i in range(24)]
            else:
                previousDateDailyBaselineValue = DailySiteReading.objects.filter(
                    associated_Site=site, reading_for=date.date()
                ).aggregate(Sum("daily_baseline_value"))
                previousDateHourlyBaselineValue = previousDateDailyBaselineValue[
                    "daily_baseline_value__sum"
                ]
                if previousDateHourlyBaselineValue is not None:
                    baselineList = [
                        round(previousDateHourlyBaselineValue / 24, 2)
                        for i in range(24)
                    ]
                else:
...
class EnergySavingsHourlyBarChartV2(APIView):
    ...
    def post(self, request):
        ...
        all_leg_ids = (
            DailySiteReading.objects.filter(associated_Site=site, reading_for=date)
            .values_list("leg_id", flat=True)
            .distinct()
        )
        print(all_leg_ids)
        ...
        hourly_data_map = {
            (item.leg_id, item.reading_from.hour): item for item in oneDayData
        }
        print(hourly_data_map)

        for leg_id in all_leg_ids:
            aisle_name_obj = AisleGroup.objects.filter(
                site=site_id, attached_leg_id=leg_id
            ).first()
        ...
        if site.is_live:
            latest_baseline = (
                SiteBaseline.objects.filter(associated_site_id=site_id)
                .exclude(Q(baseline_to__isnull=False))
            ...
            if datetime.now().date() == date.date() and latest_baseline is not None:
                ...
                baseline_list = (
                    [round(previous_baseline / 24, 2)] * 24
...
class BaselineHistory(APIView):
    ...
    def post(self, request):
        ...
        dataList = []
        dateList = []
        all_leg_id = SiteBaseline.objects.distinct("leg_id").filter(
        ...
        for i in all_leg_id:
            leg_id = i.leg_id
            data = []
            for j in SiteBaseline.objects.filter(
                associated_site_id=site_id, leg_id=leg_id
            ).order_by("baseline_from"):
...
class BaselineData(APIView):
    ...
    def post(self, request):
        ...
        for i in all_leg:
            print("leg is", i)
            aisle_info = AisleGroup.objects.filter(
                site_id=int(site_id), aisleGroupName=i.aisleGroupName
            )[0]
            print("aisle info: ", aisle_info)
            current_consumption = DailySiteReading.objects.filter(
                associated_Site=site_id, leg_id=i.attached_leg_id, reading_for=date
            )
            ...
            data = {
                "aisle_name": aisle_info.aisleGroupName,
                "total_lights": aisle_info.total_lights,
...
class SavingBaselineData(APIView):
    ...
    def post(self, request):
        ...
        aisle = AisleGroup.objects.filter(site_id=int(site_id), aisleGroupName=leg_id)[
            0
        ]
        print("aisle group is:", aisle)
        baseline_data = SiteBaseline.objects.filter(
            associated_site_id=site_id, aisle_group=aisle
        )
        print("baseline data", baseline_data)
        if baseline_data.exists():
            change_date = date - timedelta(days=1)
            update_field = SiteBaseline.objects.filter(
                associated_site_id=site, aisle_group=aisle
            ).order_by("-baseline_from")[0]
            ...
            if update_field.baseline_from == date.date():
                ...
                baseline = SiteBaseline.objects.create(
                    associated_site_id=site,
                    aisle_group=aisle,
            ...
            ).update(daily_baseline_value=baseline_value)

            print("daily table unit consumption updated", current_consumption)
            aisle_group = (
                AisleGroup.objects.distinct("aisleGroupName")
                .filter(site_id=site_id)
                .exclude(aisleGroupName=leg_id)
            )
            print("all aisle groups", aisle_group)
            for i in aisle_group:
                b = SiteBaseline.objects.filter(
                    associated_site_id=site, leg_id=i.id
                ).order_by("-baseline_from")[0]
                if b.baseline_from != date.date():
                    previous_baseline_value = b.baseline_value
                    b.baseline_to = change_date
                    b.save()
                    new_entry = SiteBaseline.objects.create(
                        associated_site_id=site,
                        leg_id=i.id,
                        aisle_group=i,
...
class AisleLevelBaseline(APIView):
    ...
    def post(self, request):
        ...
        dataList = []
        if tillDate.month == currentDate.month:
            all_leg_id = DailySiteReading.objects.distinct("leg_id").filter(
                associated_Site=site
            )
            for leg in all_leg_id:
                baseline = []
                ...
                for i in range(30):
                    ...
                    dateList.append(date.date())
                    daily = DailySiteReading.objects.filter(
                        associated_Site=site, leg_id=leg, reading_for=date
                    )
                    baseline_value = 0.0
                    if daily.exists():
                        for j in daily:
                ...
                dataList.append(
                    {"name": leg.leg_id, "data": baseline, "type": "column"}
                )
        else:
            total_days = (tillDate.day - fromDate.day) + 1
            all_leg_id = DailySiteReading.objects.distinct("leg_id").filter(
...
class TotalAlarmsOnSitePageTabularForm(APIView):
    ...
    def post(self, request):
        ...
        alarms = AlarmNotifications.objects.filter(
            created_by=customer, site_id=site, user_level=4
        )
        print("alarms: ", alarms)
        data_list = []
        for i in alarms:
        ...
        return Response(
            {"result": 1, "data": data_list, "msg": "Alarms details on site page"}
        )
...
class SiteApiBasedOnSiteType(APIView):
    ...
    def post(self, request):
        ...
        if site_type == 1:
            ...
            for i in sites:
                data_list.append(
                    {"id": i.id, "site_name": i.site_name, "site_type": i.site_type}
                )
...
class FetchAllCustomers(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"status": 200, "msg": "customer details", "data": customer_data_list}
        )
    ...
    def post(self, request):
        ...
        try:
            ...
            if user:
                ...
                return Response(
                    {"status": 200, "msg": "user created", "user_id": user.id}
                )
            return Response({"status": 400, "msg": "unable to create user"})
        except Exception as err:
            print("error", err)
            return Response(
                {"status": 500, "msg": "server error", "error": err.__class__.__name__}
            )
...
class CreateSiteUsingProvisioning(APIView):
    ...
    def post(self, request):
        ...
        if site:
            return Response(
                {"status": 200, "msg": "site created successfully", "site_id": site.id}
            )
        ...
        try:
            ...
            if site_type == 1:
                ...
                aisle_group = AisleGroup.objects.create(
                    site=site,
                    aisleGroupName=aisle_name,
                    is_this_power_source=True,
                    power_source=power_source,
                )
            else:
                aisle_group = AisleGroup.objects.create(
                    site=site, aisleGroupName=aisle_name
                )
            return Response(
                {
                    "status": 200,
                    "msg": "aisle group created successfully",
                    "aisle_id": aisle_group.id,
                }
            )
        except Exception as err:
            return Response(
...
class CreateHomeGatewayIdUsingProvisioning(APIView):
    ...
    def post(self, request):
        try:
            ...
            site_id = data.get("site_id")
            site = Site.objects.get(id=site_id)
            homeGateway = HomeGatewayId.objects.create(
                connected_to=site,
                hgw_id=home_gateway_name,
                rssh_port=rssh_port,
            ...
            if homeGateway:
                return Response(
                    {"status": 200, "msg": "home gateway id created successfully"}
                )
        except Exception as err:
            print("error : ", err)
            return Response(
...
class FireAlarm(APIView):
    ...
    def post(self, request):
        ...
        systemlist = []
        for i in all_leg:
            aisles = AisleGroup.objects.filter(
                site=site, aisleGroupName=i.aisleGroupName
            )
            print("aisles", aisles)
            ...
            if firealarm.exists:
                for j in firealarm:
                    systemlist.append(
                        {
                            "aislename": j.aisleGroup.aisleGroupName,
                            "R_Voltage": j.r_volt,
                            "Y_Voltage": j.y_volt,
                            "B_Voltage": j.b_volt,
                            "current_date": j.Updated_on,
                            "manual_time": j.manual_mode_updated_time,
                            "auto_mode": j.auto_mode_updated_time,
                            "off_mode": j.off_mode_time,
                            "motor_status": j.motor_status,
                            "motor_status_time": j.motor_status_time,
                        }
                    )
...
class EmailHistory(APIView):
    ...
    def post(self, request):
        ...
        emailhistory = (
            Email_History.objects.filter(fire_site=site)
...
class FireEquipmentSystemAdd(APIView):
    ...
    def post(self, request):
        ...
        try:
            if device_id != "":
                msg = "record update"
                FireEquipmentsSystem.objects.filter(id=int(device_id)).update(
                    deviceType=fire,
                    assetNo=assetno,
                    updatedBy=updatedby,
                    modelNo=model_no,
                    loaction=loaction,
                    Warrenty_till=datetime.strptime(warrenty_till, "%Y-%m-%d").date(),
                    last_service=datetime.strptime(last_service, "%Y-%m-%d").date(),
                    next_service=datetime.strptime(next_service, "%Y-%m-%d").date(),
                )
            else:
                msg = "record saved"
                firesystem = FireEquipmentsSystem.objects.create(
                    deviceType=fire,
                    assetNo=assetno,
                    updatedBy=updatedby,
                    modelNo=model_no,
            ...
            data_list.append(
                {
                    ...
                    "deviceName": i.deviceType.devicename,
                    "assetNo": i.assetNo,
                    "modelsNo": i.modelNo,
                    "location": i.loaction,
                    "warrentDate": i.Warrenty_till,
                    "last_serviceDate": i.last_service,
...
class FireEquiSystemTypefetch(APIView):
    ...
    def post(self, request):
        ...
        for i in fireequipment:
            list.append(
                {"row_id": i.id, "deviceName": i.devicename, "categories": i.categories}
            )
...
class FireEquiSystemTypeAdd(APIView):
    ...
    def post(self, request):
        ...
        firesystem = FireEquipmentsSystemType(
            site=site, devicename=devicename, categories=category
        )
        firesystem.save()
...
class SnapshotApi(APIView):
    ...
    def post(self, request):
        ...
        expiredDevice = FireEquipmentsSystem.objects.filter(
            next_service__lte=date
        ).count()
        warrentyDeviceList = []
        ...
        return Response(
            {
                ...
                "warrenty": warrenty,
                "expiredDevice": expiredDevice,
                "data": warrentyDeviceList,
            }
        )
    ...
    def post(self, request):
        ...
        for i in data:
            list.append(
                {
                    "areaName": i.areaName,
                    "totalLights": i.totalLights,
                    "18Watt": i.watt_18_Lights,
                    "20Watt": i.watt_20_Lights,
                    "24Watt": i.watt_24_Lights,
                    "36Watt": i.watt_36_Lights,
                    "40Watt": i.watt_40_Lights,
                    "totalWattLights": i.totalWattLights,
            ...
            list.append(
                {
                    ...
                    "80watt": i.watt_80_Lights,
                    "100watt": i.watt_100_Lights,
                    "totalWattFans": i.totalWattFans,
                    "totalUnits": i.totalUnits,
                }
            )
        return Response({"status": 200, "data": list})
...
class avgDataApi(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"status": 400, "error": "site_id must be a valid integer"}, status=400
            )
        ...
        if not from_date or not till_date:
            return Response(
                {"status": 400, "error": "from_date and till_date are required"},
                status=400,
            )
        ...
        try:
            ...
            return Response(
                {"status": 400, "error": f"Invalid date format: {str(e)}"}, status=400
            )
        ...
        try:
            ...
            return Response(
                {"status": 404, "error": f"Site with id {site_id} not found"},
                status=404,
            )
        ...
        daily = DailySiteReading.objects.filter(
            ...
            reading_for__gte=fromDate,
            leg_id__in=all_leg_id,
            reading_for__lte=tillDate,
        )
        print("daily : ", daily)
        if daily.exists():
            for i in daily:
...
class DownloadExcel(APIView):
    ...
    def post(self, request):
        ...
        if user_type == 1:
            ...
            if int(data.get("site_id", "")) == 34:
                aisles = AisleGroup.objects.filter(
                    attached_leg_id__in=[259, 276, 260, 261, 710]
                )
            else:
        ...
        for i in range(total_days):
            ...
            for aisle in all_leg_id:
                daily = DailySiteReading.objects.filter(
                    associated_Site=site_id, leg_id=aisle, reading_for=date
                )
                if daily.exists():
...
class LoadGrapghAPI(APIView):
    def post(self, request):
        ...
        try:
            ...
            current_date = datetime.now()
            raw_data = RawLoadData.objects.filter(
                site=site_id,
                ...
                res.append(
                    {
                        ...
                        "color": i.aisle_group.load_graph_color,
                    }
                )
...
class LoadGraphApiForEverySecond(APIView):
    def post(self, request):
        ...
        try:
            ...
            raw_data = RawLoadData.objects.using("secondary").filter(
                site=site_id, epoch_time__gt=epoch_time
            )
            res = []
            for i in raw_data:
                load_data = i.load_data / 1000
...
class LoadGraphApiForHourlyData(APIView):
    def post(self, request):
        ...
        try:
            ...
            hourly_data = (
                HourlyLoadData.objects.using("secondary")
                .filter(site=site_id, created__date=selected_date.date())
                .order_by("created")
            )
            res = []
...
class LoadGraphApiForDailyData(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            hourly_data = (
                DailyLoadData.objects.using("secondary")
                .filter(
                    site=site_id,
        ...
        try:
            ...
            if graph_type == "0":
                ...
                final_data = MainsDgLoadData.objects.filter(
                    site=site_id, created__date=till_date.date()
                )
            elif graph_type == "4":
                date = datetime.now()
                final_data = RawLoadData.objects.filter(
                    site=site_id, created__date=date.date(), created__hour=date.hour
                ).order_by("created")
            else:
                ...
                return Response(
                    {
                        "status": 500,
                        "data": [],
                        "msg": "No data for selected graph and date",
                    }
                )
        except Exception as err:
            print(err)
    ...
    def post(self, request):
        ...
        try:
            ...
            if pf.exists():
                for i in pf:
                    r_phase = (
                        str(round(i.r_phase_pf, 3)) + " PF Not Ok"
                        if i.r_phase_pf > 0
                        else "PF Normal"
                    )
                    y_phase = (
                        str(round(i.y_phase_pf, 3)) + " PF Not Ok"
                        if i.y_phase_pf > 0
                        else "PF Normal"
                    )
                    b_phase = (
                        str(round(i.b_phase_pf, 3)) + " PF Not Ok"
                        if i.b_phase_pf > 0
                        else "PF Normal"
                    )
                    res.append(
                        {
                            "created": i.created.strftime("%d %b, %Y %H:%M:%S"),
                            "power_source": i.supply_source,
                            "r_phase": r_phase,
                            "y_phase": y_phase,
                            "b_phase": b_phase,
                        }
...
class FetchMinMaxMonthlyLoadData(APIView):
    def post(self, request):
        ...
        try:
            site_id = data.get("id")
            monthly_load_data = MonthlyMinMaxLoadData.objects.filter(
                site=site_id
            ).order_by("created")
            res = []
            min = 100000
            ...
            if monthly_load_data.exists():
                for i in monthly_load_data:
                    ...
                    res.append(
                        {
                            "created": i.created.strftime("%b %m %Y %H:%M:%S"),
                            "power_source": i.supply_source,
                            "min_load": round(min_load, 3),
                            "min_load_created": i.min_load_created.strftime(
                                "%d %b, %Y %H:%M:%S"
                            ),
                            "max_load": round(max_load, 3),
                            "max_load_created": i.max_load_created.strftime(
                                "%d %b, %Y %H:%M:%S"
                            ),
        ...
        try:
            ...
            if monthly_load_data.exists():
                for i in monthly_load_data:
                    ...
                    min_load_date_array.append(
                        i.min_load_created.strftime("%d %b, %Y %H:%M:%S")
                    )
                    max_load_array.append(round(max_load, 3))
                    max_load_date_array.append(
                        i.max_load_created.strftime("%d %b, %Y %H:%M:%S")
                    )
                    month_year.append(i.max_load_created.strftime("%b, %Y"))
                df = pd.DataFrame(
                    {
                        "PowerSource": power_source_array,
                        "Min_Load_Value": min_load_array,
                        "Min_Load_Date": min_load_date_array,
                        "Max_Load_Value": max_load_array,
                        "Max_Load Date": max_load_date_array,
                        "month": month_year,
                    }
                )
                ...
                df = pd.DataFrame(
                    {
                        "Date": date_array,
                        "PowerSource": power_source_array,
                        "R-Phase": r_phase_array,
                        "Y-Phase": y_phase_array,
                        "B-Phase": b_phase_array,
                    }
                )
                file_name = "pf_data"
                response = HttpResponse(
...
class FetchDGAlertData(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            print(err)
            return Response(
                {"status": 500, "msg": "error while receiving alerts, {}".format(err)}
            )
    ...
    def post(self, request):
        ...
        if site:
            ...
            leg_data_qs = (
                ...
                .values("leg_id", "month")
                .annotate(total_consumption=Sum("unit_consumption"))
            )
            ...
            for aisle in aisle_group:
                ...
                final_data.append(
                    {"name": aisle_name, "data": energy_consumed_list, "type": "column"}
                )
            ...
            for m in generated_months:
                ...
                elif monthly_avg_site_level is None and (
                    m.year < live_date.year
                    or (m.year == live_date.year and m.month < live_date.month)
                ):
                    monthly_avg = 0
            ...
            final_data.append(
                {
                    ...
                    "data": monthly_avg_site_level_list,
                    "type": "spline",
                }
            )
...
class SubmeteringSnapshotAPI(APIView):
    ...
    def post(self, request):
        ...
        monthly = DailySiteReading.objects.filter(
            associated_Site=site,
            leg_id__in=all_leg_id,
            reading_for__gte=firstmnthDate,
            reading_for__lte=tillDate,
        ).aggregate(Sum("unit_consumption"))
        ...
        return Response(
            {
                "status": 200,
                "alarms": 0,
                "energy_consumed": round(energyConsumed, 2),
                "dailyAvg": avgData,
                "monthlyAvg": round(unit_consumed, 2),
                "firstmnthDate": firstmnthDate,
...
class SubmeteringHourlyBarChartNew(APIView):
    ...
    def post(self, request):
        ...
        for leg_id in legs:
            ...
            hourly_values = [
                round(data_lookup[key].get(hour, 0.0), 2) for hour in range(24)
            ]

            data_list.append({"name": name, "data": hourly_values, "type": "column"})

        end_time = datetime.now()
        print("Time taken:", end_time - start_time)

        return Response({"result": 1, "Hours": hour_list, "Data": data_list})
...
def DGfuelMonthlyTrendMultipleAisles(site_id):
    try:
        ...
        if not mains_aisles and not dg_aisles:
            return Response(
                {"status": 400, "msg": "No matching aisles found for the site."}
            )
        ...
        for i in range(11, -1, -1):
            ...
            total_mains_unit_consumed = (
                DailySiteReading.objects.filter(
                    associated_Site=site_id,
                    aisle_group__in=mains_aisles,
                    reading_for__year=month.year,
                    reading_for__month=month.month,
                ).aggregate(Sum("unit_consumption"))["unit_consumption__sum"]
                or 0
            )

            # Calculate DG unit consumption
            total_dg_unit_consumed = (
                DailySiteReading.objects.filter(
                    associated_Site=site_id,
        ...
        try:
            ...
            for i in range(11, -1, -1):
                ...
                if fuel_data.exists():
                    ...
                    last_entry = fuel_data.last()
                    last_fuel_value = last_entry.fuel_consumption
                total_dg_fuel_consumed = (
                    initial_fuel_value + refuel_value - last_fuel_value
                )
                if total_dg_fuel_consumed < 0:
                    total_dg_fuel_consumed = 0
                dg_fuel_consumption_list.append(round(total_dg_fuel_consumed, 2))
                mains_unit_consumed = DailySiteReading.objects.filter(
                    associated_Site=site_id,
                    aisle_group=mains_aisle,
                    reading_for__year=month.year,
                    reading_for__month=month.month,
                ).aggregate(Sum("unit_consumption"))
                dg_unit_consumed = DailySiteReading.objects.filter(
                    associated_Site=site_id,
                    aisle_group=dg_aisle,
                    reading_for__year=month.year,
                    reading_for__month=month.month,
                ).aggregate(Sum("unit_consumption"))
            ...
            return Response(
                {
                    "status": 200,
                    "test_Args": ["Not the 124"],
                    "months": month_list,
                    "dg_fuel_monthly": fuel,
                    "mains_unit_consumption_monthly": mains_unit_consumption_list,
                    "dg_unit_consumption_monthly": dg_unit_consumption_list,
                }
            )
        ...
        if site:
            ...
            for aisle in aisle_group:
                ...
                for i in range(11, -1, -1):
                    ...
                    # avg_consumed_list.append(round(avg_unit_consumed, 1))
                final_data.append(
                    {"name": aisleName, "data": energy_consumed_list, "type": "column"}
                )
                # avg_data.append({"name": aisleName, "data": avg_consumed_list, "type": "spline"})
            ...
            for i in range(11, -1, -1):
                ...
                # total_monthly_avg_data += monthly_avg_site_level
                if (
                    month.month == live_date.month and month.year == live_date.year
                ):  # live date month data
                    try:
                        ...
                        monthly_avg_site_level = (
                            monthly_avg_site_level / diff_from_live_date
                        ) * total_days_in_month
                        monthly_avg = monthly_avg_site_level
                    ...
                    # monthly_avg = total_monthly_avg_data / total_site_live_month
                    if (
                        month.month == today_date.month
                        and month.year == today_date.year
                    ):
                        total_days_in_month = calendar.monthrange(
                            month.year, month.month
                        )[1]
                        ...
                        monthly_avg = (
                            monthly_avg_site_level / total_days
                        ) * total_days_in_month
...
class AlarmsList(APIView):
    def post(self, request):
        ...
        high_priority = NewAlarmsNotifications.objects.filter(
            site_id=site, alarm_priority=0
        ).count()
        medium_priority = NewAlarmsNotifications.objects.filter(
            site_id=site, alarm_priority=1
        ).count()
        active_alarms = NewAlarmsNotifications.objects.filter(
            site_id=site, is_active=True
        ).count()
        alarms = NewAlarmsNotifications.objects.filter(site_id=site).distinct(
            "alarm_type"
        )
        for i in alarms:
            count = NewAlarmsNotifications.objects.filter(
                site_id=site, alarm_type=i.alarm_type
            ).count()
            data.append(
                {
                    "name": i.get_alarm_type_display(),
                    "count": count,
                    "priority": i.get_alarm_priority_display(),
...
class AlarmsListDetail(APIView):
    def post(self, request):
        ...
        try:
            ...
            alarms = NewAlarmsNotifications.objects.filter(
                site_id=site_id, alarm_type=int(alarm_type)
            )
        ...
        try:
            ...
            if tillDate.month == currentDate.month:
                ...
                all_data = DailySiteReading.objects.filter(
                    ...
                    reading_for__range=[
                        tillDate.date() - timedelta(days=29),
                        tillDate.date(),
                    ],
                    is_visible=True,
                )
                print("all_data", all_data)
                all_dates = [
                    (tillDate - timedelta(days=29 - i)).date() for i in range(30)
                ]
                print("all dates: ", all_dates)
                output = {}
                for i in all_data:
                    ...
                    if aisle_name in output:
                        if (
                            user_type in [4, 5] and previous_date.date() < i.reading_for
                        ) or user_type == 1:
                ...
                all_dates = [
                    (tillDate - timedelta(days=(total_days - 1) - i)).date()
                    for i in range(total_days)
                ]
                print("dates: ", all_dates)
                output = {}
            ...
            return Response(
                {"status": 500, "msg": str(err), "dates": all_dates, "a": a}
            )
            ...
            if tillDate.month == currentDate.month:
                ...
                for i in all_data:
                    ...
                    if (
                        ...
                        indx = (
                            all_dates.index(i.reading_for)
                            if i.reading_for in all_dates
                            else None
                        )
                        if aisle_name in output:
                            if indx is not None:
                ...
                if site.is_live:
                    if int(data.get("site_id", "")) == 34:
                        baseline = SiteBaseline.objects.filter(
                            associated_site_id=site_id,
                            leg_id__in=[259, 260, 261, 276, 710],
                        )
                    else:
                        baseline = SiteBaseline.objects.filter(
                            associated_site_id=site_id
                    ...
                    if baseline.exists():
                        if int(data.get("site_id", "")) == 34:
                            latest_baseline_value = SiteBaseline.objects.filter(
                                associated_site_id=site,
                                aisle_group__in=[259, 260, 261, 276, 710],
                            )
                        elif int(data.get("site_id", "")) == 29:
                            latest_baseline_value = SiteBaseline.objects.filter(
                                associated_site_id=29
                            ).exclude(Q(aisle_group__in=[259, 260, 261, 276, 710]))
                        else:
                            latest_baseline_value = SiteBaseline.objects.filter(
                                associated_site_id=site
                            )
...
class EnergySavingMonthlyBarChart_new(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            all_dates = [
                (till_date - timedelta(days=i)).date() for i in range(total_days)
            ]
            all_dates.reverse()
            print(all_dates)
            ...
            for record in all_data:
                ...
                if idx is not None:
                    data_dict.setdefault(aisle_name, [0] * total_days)[idx] = round(
                        unit_consumption, 2
                    )
        ...
        try:
            ...
            if download_excel:
                ...
                fuel_level_df = pd.DataFrame(
                    [
                        {
                            "Date": datetime.fromtimestamp(i["x"] / 1000).strftime(
                                "%d-%b-%Y"
                            ),
                            "Time": datetime.fromtimestamp(i["x"] / 1000).strftime(
                                "%H:%M"
                            ),
                            "Fuel_Level": i["y"],
                        }
                    ...
                    [
                        {
                            ...
                            "Unit_Consumption(KWH)": i["y"],
                            "Fuel_Consumed(Ltr)": next(
                                (f["y"] for f in dg_fuel_data if f["x"] == i["x"]), 0
                            ),
                        }
                        for i in dg_unit_data
                    ],
                ...
                for r in refuel_data:
                    alerts_data.append(
                        {
                            "Time": datetime.fromtimestamp(r["x"] / 1000).strftime(
                                "%H:%M"
                            ),
                            "Activity": "Refuel",
                            "Amount(Ltr)": r["y"],
                ...
                for t in theft_data:
                    alerts_data.append(
                        {
                            "Time": datetime.fromtimestamp(t["x"] / 1000).strftime(
                                "%H:%M"
                            ),
                            "Activity": "Drain",
                            "Amount(Ltr)": t["y"],
                ...
                alerts_df = pd.DataFrame(
                    alerts_data, columns=["Time", "Activity", "Amount(Ltr)"]
                )
        ...
        try:
            ...
            if interface_id not in map_of_interfaces.values():
                raise AttributeError(
                    "Invalid Interface ID, the Data can't be processed"
                )
            data = request.data
            DGAlertsData.objects.create(alert_data=data)
    ...
    def post(self, request):
        try:
            ...
            if user_type == 1:
                ...
                if int(data.get("site_id")) == 34:
                    aisles = AisleGroup.objects.filter(
                        attached_leg_id__in=[259, 260, 261, 710, 276]
                    )
                else:
                    aisles = AisleGroup.objects.filter(site=site_id, is_visible=True)
            ...
            for reading in readings:
                ...
                readings_dict[date][reading["leg_id"]] = round(
                    reading["total_consumption"], 2
                )
    ...
    def post(self, request):
        try:
            ...
            dg_fuel_values = (
                ...
                .annotate(Fuel_Level=Avg("fuel_consumption"))
            ...
            fuel_absolute_data = [
                {
                    ...
                    "Time": i["minute_bucket"].strftime("%H:%M"),
                    "Fuel_Level": (
                        round(i["Fuel_Level"], 2) if i["Fuel_Level"] is not None else 0
                    ),
                }
                for i in dg_fuel_values
            ]
            ...
            dg_fuel_unit_data = [
                {
                    ...
                    "DG_Unit_Consumption(KWH)": i.unit_consumption,
                    "DG_Unit_Per_Ltr": (
                        round(i.unit_consumption / i.dg_fuel_consumption, 2)
                        if i.dg_fuel_consumption > 0
                        else 0
                    ),
                }
            ...
            with pd.ExcelWriter(io_buffer, engine="xlsxwriter") as writer:
                if fuel_absolute_data:
                    pd.DataFrame(fuel_absolute_data).to_excel(
                        writer, sheet_name="Fuel Level", index=False
                    )
                if dg_fuel_unit_data:
                    pd.DataFrame(dg_fuel_unit_data).to_excel(
                        writer, sheet_name="DG Fuel & Unit Consumption", index=False
                    )
                if refuel_theft_data:
                    pd.DataFrame(refuel_theft_data).to_excel(
                        writer, sheet_name="DG Refuel & Theft Data", index=False
                    )
            ...
            response["Content-Disposition"] = (
                f"attachment; filename=Report_DG_Fuel_Unit_Trend_{current_date}.xlsx"
            )
            return response
...
class PowerDistributionPieChartV2(APIView):
    ...
    def _parse_month_year(data):
        current_date = datetime.now()
        month = data.get("month") or data.get("selected_month")
        year = data.get("year") or data.get("selected_year")

        try:
            month = int(month) if month not in (None, "") else current_date.month
            year = int(year) if year not in (None, "") else current_date.year
        ...
        if year < 1:
            return None, None, "year must be a valid year"
    ...
    def post(self, request):
        try:
            ...
            if not site_id:
                return Response(
                    {"error": "SiteID is required"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            try:
                ...
                return Response(
                    {"error": "SiteID must be a valid integer"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            month, year, parse_error = self._parse_month_year(request.data)
            if parse_error:
                return Response(
                    {"error": parse_error}, status=status.HTTP_400_BAD_REQUEST
                )

            base_readings = DailySiteReading.objects.filter(
                associated_Site_id=site_id,
                reading_for__isnull=False,
            )
            ...
            available_months = [
                ...
                for item in base_readings.annotate(month=TruncMonth("reading_for"))
                .values("month")
                .distinct()
                .order_by("-month")
            ]
            ...
            readings = list(
                base_readings.filter(reading_for__year=year, reading_for__month=month)
                .values("leg_id")
            ...
            aisle_groups = {
                ...
                for ag in AisleGroup.objects.filter(
                    site_id=site_id, attached_leg_id__in=leg_ids
                ).only("attached_leg_id", "aisleGroupName")
            }
            ...
            for r in readings:
                ...
                source_totals[source_name] = source_totals.get(source_name, 0) + (
                    r["total_units"] or 0
                )

            total_sum = sum(source_totals.values())
            finalData = []

            for name, total in source_totals.items():
                percentage = total * 100 / total_sum if total_sum else 0
                finalData.append({"name": name, "data": round(percentage, 2)})

            selected_date = datetime(year=year, month=month, day=1)
            previous_month_date = selected_date - timedelta(days=1)
            previous_month_available = any(
                item["month"] == previous_month_date.month
                and item["year"] == previous_month_date.year
...
class PowerSource(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": str(err), "message": "Internal Server Error"}, status=500
            )
        ...
        try:
            ...
            hourlyLoadData = (
                HourlyLoadData.objects.filter(
                    aisle_group_id=data.get("aisle_id"),
                    created__date=selected_date.date(),
                )
                .order_by("created")
                .values("epoch_time", "load_data", "aisle_group__load_graph_color")
            )
...
class LoadDataApiMultipleSourceV2(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            response_data = [
                {
                    group["aisleGroupName"]: filled_grouped_data.get(
                        group["aisleGroupName"], []
                    )
                }
                for group in aisle_groups
            ]
...
class LiveDataApi(APIView):
    ...
    def post(self, request):
        ...
        try:
            load_data = RawLoadData.objects.filter(
                aisle_group__site_id=site_id, epoch_time__gt=epoch_time
            ).prefetch_related("aisle_group")
            grouped_data = defaultdict(list)
            for obj in load_data:
                grouped_data[obj.aisle_group.aisleGroupName].append(
                    {"load": obj.load_data / 1000, "epoch": obj.epoch_time}
                )
...
class DgFuelMonthlyTrend_test(APIView):
    ...
    def post(self, request):
        try:
            ...
            month_list = [
                (current_date - relativedelta(months=i)) for i in range(11, -1, -1)
            ]
            ...
            dg_fuel_map = {
                row["month"].strftime("%b-%Y"): row["total"] or 0 for row in dg_fuel_raw
            }
            ...
            mains_units_map = {
                row["month"].strftime("%b-%Y"): row["total"] or 0
                for row in mains_units_raw
            }
            ...
            dg_units_map = {
                row["month"].strftime("%b-%Y"): row["total"] or 0
                for row in dg_units_raw
            }

            # ---- Build final aligned lists ----
            dg_fuel_consumption_list = [
                round(dg_fuel_map.get(label, 0), 2) for label in month_labels
            ]
            mains_unit_consumption_list = [
                round(mains_units_map.get(label, 0), 2) for label in month_labels
            ]
            dg_unit_consumption_list = [
                round(dg_units_map.get(label, 0), 2) for label in month_labels
            ]
...
class LoadDataExcelDownloadMonthlyRange(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                site = Site.objects.get(id=site_id)
                site_name = site.site_name.replace(" ", "").replace(
                    "/", ""
                ...
                for i in final_data:
                    ...
                    if load_data > 0:
                        if hasattr(i, "created"):
                            ...
                            date_array.append(
                                i.created_on.strftime("%d %b, %Y %H:%M:%S")
                            )

                        data_array.append(round(load_data, 3))

                        if (graph_type == "3" or graph_type == "4") and hasattr(
                            i, "aisle_group"
                        ):
                            power_source.append(i.aisle_group.aisleGroupName)
...
#     def post(self, request):
#         hourly_response = EnergySavingsHourlyBarChartVB().post(request)

#         if hourly_response.status_code != status.HTTP_200_OK or hourly_response.data.get("result") != 1:
#             return hourly_response

#         try:
#             site_id = request.data.get("site_id")
#             date = datetime.strptime(request.data.get("date", ""), "%Y/%m/%d")
#             site = Site.objects.only("site_name").get(id=site_id)

#             hours = hourly_response.data["Hours"]
#             consumption_series = hourly_response.data["Data"]

#             baseline = next(
#                 (series["data"] for series in consumption_series if series["name"].lower() == "baseline"),
#                 [0] * len(hours),
#             )
#             aisle_consumption = [
#                 series for series in consumption_series if series["name"].lower() != "baseline"
#             ]
#             aisle_names = [series["name"] for series in aisle_consumption]
#             header = [
#                 "Hour",
#                 "Baseline",
#                 "Savings",
...
#                 aisle_values = [series["data"][index] for series in aisle_consumption]
#                 total = round(sum(aisle_values), 2)
#                 baseline_value = round(baseline[index], 2)
#                 data_rows.append([
#                     hour,
#                     baseline_value,
#                     round(baseline_value - total, 2),
#                     total,
#                 ] + aisle_values)

#             df = pd.DataFrame(data_rows, columns=header)
#             site_name = site.site_name.replace(" ", "_").replace("/", "_")
#             file_name = f"{site_name}_hourly_energy_{date.strftime('%Y-%m-%d')}.csv"
#             response = HttpResponse(content_type="text/csv")
#             response["Content-Disposition"] = f'attachment; filename="{file_name}"'
#             df.to_csv(response, index=False)
#             return response
...
#         except Site.DoesNotExist:
#             return Response({"result": 0, "msg": "Site not found"}, status=status.HTTP_404_NOT_FOUND)
#         except Exception as err:
#             return Response(
#                 {"result": 0, "msg": str(err)},
...
class EnergySavingsHourlyExcelDownloadVB(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if not site_id:
                return Response(
                    {"result": 0, "msg": "site_id is required"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"result": 0, "msg": "site_id must be an integer"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            from_date_str = (
                data.get("from_date") or data.get("from") or data.get("date") or ""
            ).strip()
            end_date_str = (
                data.get("end_date")
                or data.get("to_date")
                or data.get("to")
                or data.get("till_date")
                or ""
            ).strip()
            ...
            if not from_date_str:
                return Response(
                    {
                        "result": 0,
                        "msg": "Date parameter is required (from_date/from/date)",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if not end_date_str:
                end_date_str = from_date_str
            ...
            def parse_date(d_str):
                for fmt in ("%Y/%m/%d", "%Y-%m-%d", "%d-%b-%Y"):
                    try:
                        ...
                        pass
                raise ValueError(
                    f"Time data '{d_str}' does not match format '%Y/%m/%d', '%Y-%m-%d', or '%d-%b-%Y'"
                )
            ...
            try:
                ...
                return Response(
                    {"result": 0, "msg": str(e)}, status=status.HTTP_400_BAD_REQUEST
                )

            if from_date > end_date:
                return Response(
                    {"result": 0, "msg": "from_date cannot be after end_date"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if (end_date - from_date).days > 31:
                return Response(
                    {"result": 0, "msg": "Date range cannot exceed 31 days"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            for item in range_data:
                r_datetime = item["reading_from"]
                if not r_datetime:
                    continue
                r_date = r_datetime.date()
                r_hour = r_datetime.hour
                hourly_data_map[(r_date, str(item["leg_id"]), r_hour)] = item

            # Fetch baselines for each date in the range in-memory to prevent multiple DB queries or reliance on corrupt DailySiteReading values
            baselines = []
            if site.is_live:
                baselines = list(
                    SiteBaseline.objects.filter(
                        associated_site_id=site_id, leg_id__in=str_leg_ids
                    )
                )

            # Helper to calculate daily baseline value for any date in the range
            def get_daily_baseline_for_date(d):
                if not site.is_live:
                    return 0.0
                total_val = 0.0
                for b in baselines:
                    from_ok = b.baseline_from is None or b.baseline_from <= d
                    to_ok = b.baseline_to is None or b.baseline_to >= d
                    if from_ok and to_ok:
                        total_val += b.baseline_value or 0.0
                return total_val

            # Generate dates in the range
            total_days = (target_date_to - target_date_from).days + 1
            date_list = [
                target_date_from + timedelta(days=i) for i in range(total_days)
            ]

            # Data structures for Excel
            hourly_rows = []

            for d in date_list:
                # Baseline for the day
                day_total_baseline = get_daily_baseline_for_date(d)

                # Daily accumulation variables
                day_hourly_rows = []

                for h in range(24):
                    hour_str = "{:02d}:00".format(h)
                    baseline_value = (
                        round(day_total_baseline / 24.0, 2)
                        if day_total_baseline
                        else 0.0
                    )

                    aisle_values = []
                    for leg_id in sorted_leg_ids:
                        hourly_data = hourly_data_map.get((d, str(leg_id), h))
                        if hourly_data:
                            val = hourly_data["unit_consumption"] or 0.0
                        else:
                            val = 0.0
                        aisle_values.append(round(val, 2))

                    total_consumption = round(sum(aisle_values), 2)
                    if site.is_live:
                        savings = round(baseline_value - total_consumption, 2)
                    else:
                        savings = 0.0

                    day_hourly_rows.append(
                        [d, hour_str, baseline_value, savings, total_consumption]
                        + aisle_values
                    )

                hourly_rows.extend(day_hourly_rows)
            ...
            df_hourly = pd.DataFrame(hourly_rows, columns=hourly_headers)

            # Create Excel workbook in-memory
            output = BytesIO()
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                df_hourly.to_excel(writer, sheet_name="Hourly Readings", index=False)
                ...
                for sheet_name in ["Hourly Readings"]:
                    ...
                    for row_num in range(2, worksheet.max_row + 1):
                        for col_num in range(1, worksheet.max_column + 1):
                            ...
                            if col_num == 1:
                                ...
                                cell.alignment = alignment_center
                            elif sheet_name == "Hourly Readings" and col_num == 2:
                                cell.alignment = alignment_center
                            else:
            ...
            output.seek(0)
            site_name_clean = re.sub(r"[^a-zA-Z0-9_-]", "_", site.site_name)
            file_name = f"{site_name_clean}_hourly_energy_{target_date_from.strftime('%Y-%m-%d')}_to_{target_date_to.strftime('%Y-%m-%d')}.xlsx"
...
class MeterDisconnectionListView(APIView):
    def get(self, request):
        ...
        if logged_site_id:
            queryset = queryset.filter(logged_site_id=logged_site_id)
        if meter_number:
            queryset = queryset.filter(meter_number=meter_number)
        if meter_id:
        ...
        try:
            ...
            return Response(
                {"status": 400, "msg": "limit and offset must be numbers."}, status=400
            )

        limit = max(1, min(limit, 500))
        offset = max(0, offset)
        serializer = MeterDisconnectionEventSerializer(
...
class EnergySavingsHourlyBarChartVB(APIView):
    ...
    def post(self, request):
        ...
        if site.is_live:
            latest_baseline = (
                SiteBaseline.objects.filter(aisle_group__virtual_siteID_id=site_id)
                .exclude(Q(baseline_to__isnull=False))
                .aggregate(Sum("baseline_value"))["baseline_value__sum"]
            )
    ...
    def post(self, request):
        try:
            ...
            if user_type == 1:
                ...
                aisles = AisleGroup.objects.filter(
                    virtual_siteID_id=site_id, is_visible=True
                )

            aisle_map = {
                aisle.attached_leg_id: aisle.aisleGroupName for aisle in aisles
            }
            leg_ids = list(aisle_map.keys())

            readings = DailySiteReading.objects.filter(
                aisle_group__virtual_siteID_id=site_id,
                leg_id__in=leg_ids,
                reading_for__range=[start_date.date(), end_date.date()],
                is_visible=True,
            ).values("leg_id", "reading_for", "unit_consumption")

            total_days = (end_date - start_date).days + 1
            all_dates = [
                (start_date + timedelta(days=i)).date() for i in range(total_days)
            ]

            data_dict = {}

            for reading in readings:
                leg_id = reading["leg_id"]
                reading_for = reading["reading_for"]
                unit_consumption = reading["unit_consumption"]
            ...
            aisle_names = list(data_dict.keys())
            ...
            for idx, current_date in enumerate(all_dates):
                baseline_value = (
                    latest_baseline_values.filter(baseline_from__lte=current_date)
                    .filter(
                        Q(baseline_to__gte=current_date) | Q(baseline_to__isnull=True)
                    )
                    .aggregate(Sum("baseline_value"))["baseline_value__sum"]
                    or 0
                )

                row = [current_date.strftime("%d-%b-%Y")]
                total = 0

                for aisle_name in aisle_names:
                    consumption = data_dict[aisle_name][idx]
                    total += consumption
                    row.append(consumption)

                row.insert(1, round(total, 2))
        ...
        try:
            ...
            if site_id:
                ...
                try:
                    customer = User.objects.get(id=int(user_id))
                    if customer.UserType == 5:
                        user_sites = CustomerSiteManager.objects.get(
                            customer=customer
                        ).associated_site.all()
                    else:
                        user_sites = Site.objects.filter(customer=customer)
...
class SiteAisleGroupsWithLastEntries(APIView):
    ...
    def _get_latest_hourly_entries(self, site_ids, leg_ids, required_keys):
        ...
        hourly_entries = (
            HourlySiteReading.objects.filter(
                associated_Site_id__in=site_ids, leg_id__in=leg_ids
            )
            .values(
                "id",
                "associated_Site_id",
    ...
    def _build_response(self, request):
        site_id = (
            ...
            else request.query_params.get("site_id")
        )
        ...
        aisle_groups = list(
            AisleGroup.objects.filter(site_id__in=site_ids).order_by("site_id", "id")
        )
        ...
        if required_keys and leg_ids:
            latest_hourly_entries = self._get_latest_hourly_entries(
                site_ids, leg_ids, required_keys
            )
            latest_daily_entries = self._get_latest_daily_entries(
                site_ids, leg_ids, required_keys
            )
...
class ConsumptionAlarmsAPI(APIView):
    def post(self, request):
        data = request.data
        try:
            site_id = int(data.get("site_id", 0))
            ...
            alarms_qs = AlarmNotifications.objects.filter(
                site_id=site_id, Alarm_type__in=[7, 8], is_active=True
            )
            ...
            alarms_qs = alarms_qs.order_by("-created_time")

            response_data = []
            for alarm in alarms_qs:
                aisle_name = (
                    alarm.aisle_group.aisleGroupName
                    if hasattr(alarm, "aisle_group") and alarm.aisle_group
                    else str(alarm.object_id) if alarm.object_id else ""
                )
                response_data.append(
                    {
                        "id": alarm.id,
                        "alarm_type": alarm.get_Alarm_type_display(),
                        "priority": alarm.get_Alarm_priority_display(),
                        "aisle_group": aisle_name,
                        "created_time": alarm.created_time.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    }
                )
...
class MeterDisconnectionIngestView(APIView):
    authentication_classes = []
    permission_classes = []
    ...
    def post(self, request):
        ...
        if not isinstance(payload, list):
            return Response(
                {"status": 400, "msg": "Payload must be object or list."}, status=400
            )
        ...
        for entry in payload:
            ...
            defaults = {
                "remote_site_id": entry.get("site"),
                "logged_site_id": entry.get("logged_site_id"),
                "meter_source_id": entry.get("meter_source"),
                ...
                "source_log_path": entry.get("source_log_path") or "",
                "raw_log_line": entry.get("raw_log_line"),
                "synced_to_cloud": True,
                "synced_at": (
                    parse_datetime(entry.get("synced_at") or "")
        ...
        return Response({"status": 200, "saved": saved, "received": len(payload)})
