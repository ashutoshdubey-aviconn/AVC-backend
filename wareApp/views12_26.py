                ...
                if previous_month_year <= month_year:
                    ...
                    if previous_date.month == month.month:
                        ...
                        if user_type == 1:
                            ...
                            # aisle_group = AisleGroup.objects.filter(site_id=site_id)
                            all_leg_id = [
                                aisle.attached_leg_id for aisle in aisle_group
                            ]
                            print("All legs", all_leg_id)
                            daily = DailySiteReading.objects.filter(
                                leg_id__in=all_leg_id,
...
class FetchAllSitesForProvisoning(APIView):
    ...
    def get(self, request):
        ...
        for i in all_sites:
            site_list.append(
                {"id": i.id, "site_name": i.site_name, "location": i.location}
            )
...
class EmailHistory(APIView):
    ...
    def post(self, request):
        ...
        for i in all_leg:
            ...
            print("aisles", aisles)
            emailhistory = Email_History.objects.filter(
                fire_site=site, deviceName__in=aisles
            ).order_by("-created")
            for j in emailhistory:
                email_list.append(
                    {
                        "site": j.fire_site.site_name,
                        "device_name": j.deviceName.aisleGroupName,
                        "email_for": j.email_for,
                        "datetime": j.created,
                    }
                )
...
class LoadGraphApiForDailyData(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            hourly_data = DailyLoadData.objects.filter(
                site=site_id,
    ...
    def post(self, request):
        ...
        try:
            ...
            for aisle in all_aisles:
                hourly_data = MainsDgLoadData.objects.filter(
                    site=site_id, aisle_group=aisle.id, created__date=till_date.date()
                ).order_by("created")
            ...
            for i in refueling_alerts:
                epoch_time = (
                    ...
                    if i.get("event_time", "") != ""
                        ...
                        i.get("dateTimeStamp", "")[:-6], "%Y-%m-%dT%H:%M:%S"
                    ).timestamp()
                )
                print(epoch_time)
                refuel_data.append(
                    {
                        ...
                        "y": float(
                            i.get(
                                "refueled_in_liters",
                                i.get("fuelChange", {}).get("fuel_change", ""),
                            )
                        ),
                    }
                )
...
class EnergySavingsHourlyBarChartVB(APIView):
    ...
    def post(self, request):
        ...
        for leg_id in all_leg_ids:
            aisle_name_obj = AisleGroup.objects.filter(
                virtual_siteID_id=site_id, attached_leg_id=leg_id
            ).first()
            name = aisle_name_obj.aisleGroupName if aisle_name_obj else leg_id
            print("name is", name)
            unitConsumptionList = []
        ...
        if site.is_live:
            latest_baseline = (
                SiteBaseline.objects.filter(aisle_group__virtual_siteID_id=site)
                .exclude(Q(baseline_to__isnull=False))
                .aggregate(Sum("baseline_value"))["baseline_value__sum"]
            )
            ...
            if datetime.now().date() == date.date() and latest_baseline is not None:
                ...
                previous_baseline = DailySiteReading.objects.filter(
                    aisle_group__virtual_siteID_id=site, reading_for=date.date()
                ).aggregate(Sum("daily_baseline_value"))["daily_baseline_value__sum"]
