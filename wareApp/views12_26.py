...
class EnergySavingMonthlyTrendApi(APIView):
    ...
    def post(self, request):
        ...
        return Response({"result": 1, "months": month_list, "energySaved": energy_saved_list,
                         "energyConsumed": energy_consumed_list, "carbon_saved": carbon_list,
                         "percentageSaved": percentage_saved_list})
...
class DgFuelConsumptionDataApiUsingLoconavAPI_new(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            refueling_alerts = [i.alert_data for i in refuel_alerts_for_site]
            for i in refueling_alerts:
                epoch_time = datetime.strptime(i.get('event_time','')[:-6], "%Y-%m-%dT%H:%M:%S.%f").timestamp() if i.get('event_time','') != '' else datetime.strptime(i.get('dateTimeStamp','')[:-6], "%Y-%m-%dT%H:%M:%S").timestamp()
                print(epoch_time)
                refuel_data.append({'x': int(epoch_time) * 1000, 'y': float(i.get('refueled_in_liters', i.get('fuelChange', {}).get('fuel_change','')))})
            ...
            for i in theft_alerts:
                ...
                theft_data.append({'x': int(epoch_time * 1000), 'y': float(i.get('value', i.get('fuelChange', '').split('ltr')[0]))})
