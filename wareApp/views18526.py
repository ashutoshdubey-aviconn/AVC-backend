        ...
        for users in all_users:
            ...
            if sum(total_energy_saved) > 0 and sum(total_baseline) > 0:
                try:
                    ...
                    max_saving = (
                        max(total_energy_saved)
                        / (
                            total_baseline[
                                total_energy_saved.index(max(total_energy_saved))
                            ]
                        )
                    ) * 100
                    min_saving = (
                        min(total_energy_saved)
                        / (
                            total_baseline[
                                total_energy_saved.index(min(total_energy_saved))
                            ]
                        )
                    ) * 100
...
class PowerDistributionPieChart(APIView):
    ...
    def post(self, request):
        ...
        for leg in DailySiteReading.objects.distinct("leg_id").filter(
            ...
            print("inside first for loop")
            graph = DailySiteReading.objects.filter(
                associated_Site=site_id,
                leg_id=leg,
                reading_for__year=current_date.year,
                reading_for__month=current_date.month,
...
class PowerDistributionPieChartV2(APIView):
    ...
    def post(self, request):
        try:
            ...
            # 3. Build datalist
            datalist = [
                {"name": name, "data": total} for name, total in source_totals.items()
            ]
            ...
            for item in datalist:
                ...
                finalData.append({"name": item["name"], "data": round(percentage, 2)})
...
class SiteCeleryPingApi(APIView):
    ...
    def post(self, request):
        try:
                ...
                response_data.append(
                    {
                        ...
                        "celery_ping_time": (
                            p.celery_ping_time.strftime("%Y-%m-%d %H:%M:%S")
                            if p.celery_ping_time
                            else None
                        ),
                        "updated_on": (
                            p.updated_on.strftime("%Y-%m-%d %H:%M:%S")
            ...
            response_data.sort(
                key=lambda x: 0 if x.get("unit_consumption") == 0.0 else 1
            )
            return Response({"result": 1, "data": response_data})
        except Exception as err:
            return Response({"result": 0, "msg": str(err)})
