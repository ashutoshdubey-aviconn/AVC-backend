...
from wareApp.fuel_providers import fetch_roadcast_fuel
...
def roadcasteAPIUpdated(vehical, date):
    try:
        ...
        # convert date to start/end datetimes covering the day
        if isinstance(date, _dt):
            sdt = date
            edt = date
        else:
            try:
                sdt = _dt.combine(date, _time(0, 0, 0))
                edt = _dt.combine(date, _time(23, 59, 59))
            except Exception:
                sdt = _dt.now()
                edt = _dt.now()
            ...
            try:
                ...
                if site.site_type == 1 and site.dg_fuel_system_installed:
                    try:
                        ...
                        # filter DG unit consumption by the message date (avoid hard-coded dates)
                        ...
                        if dg_unit.exists():
                            ...
                            if provider == "roadcast":
                                ...
                                if (
                                    ...
                                    headers = {
                                        "User-Authentication": "51uKh_YaL72s7zhx6bwZ"
                                    }
                                    ...
                                    if "data" in fetch_fuel:
                                        ...
                                        if "fuel_consumption" in fetch_fuel:
                                            fuel = fetch_fuel["fuel_consumption"][
                                                "value"
                                            ]
                                        ...
                                        if last_entry.is_dg_on:
                                            ...
                                            if "data" in fetch_fuel:
                                                ...
                                                if "fuel_consumption" in fetch_fuel:
                                                    ...
                                                    last_record.dg_fuel_consumption = (
                                                        fuel
                                                    )
                                                    last_record.save()
                                                    print(
