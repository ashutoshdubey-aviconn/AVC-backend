...
class SiteCeleryPingApi(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Sort: zero consumption first, then by celery_ping_time descending (newest first)
            response_data.sort(key=lambda x: (0 if x.get("unit_consumption") == 0.0 else 1, x.get("celery_ping_time") or ""), reverse=False)
            # Re-sort to get newest timestamps first within each consumption group
            response_data.sort(key=lambda x: (0 if x.get("unit_consumption") == 0.0 else 1, -(ord(x.get("celery_ping_time")[0]) if x.get("celery_ping_time") else 0) if x.get("celery_ping_time") else 0))
            
            # Better approach: separate into two lists and combine
            zero_consumption = [x for x in response_data if x.get("unit_consumption") == 0.0]
            non_zero_consumption = [x for x in response_data if x.get("unit_consumption") != 0.0]
            
            zero_consumption.sort(key=lambda x: x.get("celery_ping_time") or "", reverse=True)
            non_zero_consumption.sort(key=lambda x: x.get("celery_ping_time") or "", reverse=True)
            
            response_data = zero_consumption + non_zero_consumption
            return Response({"result": 1, "data": response_data})
        except Exception as err:
            return Response({"result": 0, "msg": str(err)})
